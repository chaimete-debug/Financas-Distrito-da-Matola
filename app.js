...
function formatDateOnly(v){
  ...
  // Se já vier como yyyy-MM-dd, mantém simples.
  if(/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
  // Se vier como ISO do Apps Script, remove hora e ajusta ao fuso local do navegador.
  const d = new Date(s);
  if(!isNaN(d.getTime())){
  ...
  // Fallback para valores em formato textual.
...
async function serverCall(fnName,args,onSuccess,onFailure){
  ...
  const noCache = [
    ...
    'Lancamento_submit',
    'Approval_decide',
    'Approval_listPending',
    'Approval_repararPendencias',
    'User_create',
    'User_toggleActive',
    'Igreja_create',
    'Igreja_toggleActive',
...
function carregarLancamentos(){
  ...
  serverCall('Lancamento_listAll',[filtros],function(res){
    ...
    wrap.innerHTML=`<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table>
      ...
      <tbody>${rows.map(l=>`<tr>
        <td style="white-space:nowrap;">${formatDateOnly(l.data_movimento)}</td>
...
function carregarAprovacoes(){
  ...
  serverCall('Approval_listPending',[],function(res){
    ...
    wrap.innerHTML=`<div style="overflow-x:auto;-webkit-overflow-scrolling:touch;"><table>
      ...
      <tbody>${rows.map(l=>`<tr>
        <td style="white-space:nowrap;">${formatDateOnly(l.data_movimento||l.data_lancamento)}</td>
        <td>${safeText(l.descricao)}</td>
        <td>${nomeGrupo(l.id_grupo)}</td>
        <td>${nomeRubrica(l.id_rubrica)}</td>
        <td>${nomeDepto(l.id_departamento)}</td>
...
function gerarRelatorioRubricas(){
  ...
  // V12: o relatório por rubrica passa a ser calculado no backend,
  // para respeitar o mesmo âmbito e a mesma perspectiva contabilística
  // usada no Dashboard (local vê Saída; distrito vê Entrada).
  serverCall('Dashboard_resumoRubricas',[{mes:mesInicio, mesInicio, mesFim, ano, periodo:'livre'}],function(res){
    const linhas=Array.isArray(res?.linhas)?res.linhas:[];
    const igNome=res?.igreja_nome?` — ${res.igreja_nome}`:'';
    if(!linhas.length){ wrap.innerHTML=`<div class="info">ℹ️ Sem lançamentos aprovados em ${label}${igNome}.</div>`; return; }
...
function saldosFormDeptos(){
  const wrap = document.getElementById('saldosContentWrap');
  if (!wrap) return;
  ...
  getOpts(function(opts){
    const deptos = (opts?.departamentos || [])
      .filter(d => String(d.estado || 'ACTIVO').toUpperCase() !== 'INACTIVO');
    const igrejas = opts?.igrejas || [];

    if (!deptos.length) {
      wrap.innerHTML = '<div class="info">ℹ️ Sem departamentos activos. Crie departamentos primeiro.</div>';
      return;
    }
    ...
    for (let a = ano; a >= ano - 5; a--) {
      anosH += `<option value="${a}"${a === ano ? ' selected' : ''}>${a}</option>`;
    }

    const igrejaOpts = '<option value="">Global / Todos</option>' +
      igrejas.map(i => `<option value="${String(i.id_igreja || '')}">${safeText(i.nome_igreja)}</option>`).join('');

    const deptoIds = deptos.map(d => String(d.id_departamento || '')).filter(Boolean);
    window.__saldosDeptosIds = deptoIds;
    ...
    wrap.innerHTML = `
      ...
      <div id="msgSaldoD" class="msg" style="display:none;margin-bottom:8px;"></div>
      ...
      <div style="overflow-x:auto;">
        <table id="tblSaldoDeptos">
          ...
          <tbody>
            ${deptos.map(d => {
              const id = String(d.id_departamento || '');
              return `
              <tr>
                <td>
      ...
      <div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap;">
        <button type="button" id="btnLimparSaldosDeptos" class="secondary" style="width:auto;margin-top:0;padding:11px 22px;">🧹 Limpar Campos</button>
        <button type="button" id="btnGuardarSaldosDeptos" class="success" style="width:auto;margin-top:0;padding:11px 26px;">💾 Guardar Saldos de Departamentos</button>
      </div>
      <div id="msgSaldoD2" class="msg" style="display:none;margin-top:10px;"></div>
      <p class="muted" style="margin-top:10px;font-size:12px;">Deixe o campo em branco para não alterar o saldo desse departamento. Para gravar zero, escreva 0.</p>`;

    const btnLimpar = document.getElementById('btnLimparSaldosDeptos');
    const btnGuardar = document.getElementById('btnGuardarSaldosDeptos');

    if (btnLimpar) {
      btnLimpar.addEventListener('click', function(){
        saldosLimparFormDeptos(window.__saldosDeptosIds || []);
      });
    }
    ...
    if (btnGuardar) {
      btnGuardar.addEventListener('click', function(e){
        e.preventDefault();
        e.stopPropagation();
        saldosGuardarDeptos(window.__saldosDeptosIds || []);
      });
    }

    saldosPreencherDeptos(deptoIds);
  });
}
...
function saldosPreencherDeptos(ids){
  serverCall('SaldoInicial_listDeptos', [], function(res){
    ...
    lista.forEach(function(item){
      const id = String(item.id_departamento || '');
      const vid = document.getElementById('sid_val_' + id);
      const oid = document.getElementById('sid_obs_' + id);

      const valor = item.saldo_inicial !== undefined && item.saldo_inicial !== null && item.saldo_inicial !== ''
        ? item.saldo_inicial
        : item.valor_inicial;

      if (vid && valor !== undefined && valor !== null && valor !== '') vid.value = String(valor).replace('.', ',');
      if (oid && (item.observacao || item.observacoes)) oid.value = item.observacao || item.observacoes || '';
    });
  }, function(err){
    saldosMostrarMsgDeptos_('⚠️ Não foi possível carregar saldos já existentes: ' + (err?.message || 'erro desconhecido'), false);
  });
}
...
function saldosLimparFormDeptos(ids){
  ...
  saldosMostrarMsgDeptos_('🧹 Campos limpos.', true);
}

function saldosGuardarDeptos(ids){
  try {
    ids = Array.isArray(ids) ? ids : (window.__saldosDeptosIds || []);
    ...
    const btn = document.getElementById('btnGuardarSaldosDeptos');
    ...
    ids.forEach(function(id){
      ...
      // Campo vazio: não altera esse departamento.
      if (raw === '') return;

      const val = saldosParseNumero_(raw);
      if (isNaN(val)) {
        throw new Error('Valor inválido no departamento ' + id + ': ' + raw);
      }
      ...
      entradas.push({
        ...
        ano: ano,
        id_igreja: igreja,
        observacao: obsInput ? String(obsInput.value || '').trim() : '',
        observacoes: obsInput ? String(obsInput.value || '').trim() : ''
      });
    });

    if (!entradas.length) {
      saldosMostrarMsgDeptos_('⚠️ Não foram introduzidos valores válidos. Preencha pelo menos um saldo.', false);
      return;
    }
    ...
    saldosMostrarMsgDeptos_(`⏳ A guardar ${entradas.length} saldo(s) de departamento...`, true);

    serverCall(
      'SaldoInicial_saveBulk',
      [{ departamentos: entradas, rubricas: [] }],
      function(res){
        invalidarCache();
        saldosMostrarMsgDeptos_('✅ Saldos de departamentos guardados com sucesso.', true);
        if (btn) {
          btn.disabled = false;
          btn.textContent = '💾 Guardar Saldos de Departamentos';
        }
        setTimeout(function(){ saldosPreencherDeptos(ids); }, 500);
      },
      function(err){
        saldosMostrarMsgDeptos_('❌ Erro ao guardar saldos: ' + (err?.message || 'erro desconhecido'), false);
        if (btn) {
          btn.disabled = false;
          btn.textContent = '💾 Guardar Saldos de Departamentos';
        }
    ...
    saldosMostrarMsgDeptos_('❌ Erro no formulário: ' + (e?.message || e), false);
    const btn = document.getElementById('btnGuardarSaldosDeptos');
    if (btn) {
      btn.disabled = false;
      btn.textContent = '💾 Guardar Saldos de Departamentos';
...
function saldosParseNumero_(raw){
...
function saldosMostrarMsgDeptos_(texto, ok){
  ['msgSaldoD', 'msgSaldoD2'].forEach(function(id){
