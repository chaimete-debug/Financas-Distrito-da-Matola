  ...
  getOpts(function(opts){
    ...
    wrap.innerHTML=`
      ...
      <div style="margin-top:16px;display:flex;gap:10px;flex-wrap:wrap;">
        ...
        <button id="btnGuardarSaldosDeptos" class="success" style="width:auto;margin-top:0;padding:11px 26px;" onclick="saldosGuardarDeptos(${JSON.stringify(deptos.map(d=>d.id_departamento))})">💾 Guardar Saldos de Departamentos</button>
      </div>
      <div id="msgSaldoD2" class="msg" style="margin-top:10px;"></div>
...
function saldosGuardarDeptos(ids){
  ...
  function msg(texto, ok){
    showMessage(texto, ok, 'msgSaldoD');
    showMessage(texto, ok, 'msgSaldoD2');
  }
  ...
  if (!entradas.length) {
    msg('⚠️ Não foram introduzidos valores válidos. Preencha pelo menos um saldo.', false);
    return;
  }
  ...
  msg(`⏳ A guardar ${entradas.length} saldo(s) de departamento...`, true);
  ...
  serverCall(
    ...
    function(res){
      invalidarCache();
      msg('✅ Saldos de departamentos guardados com sucesso.', true);
      if (btn) {
        btn.disabled = false;
    ...
    function(err){
      msg('❌ Erro ao guardar saldos: ' + (err?.message || 'erro desconhecido'), false);
      if (btn) {
        btn.disabled = false;
        btn.textContent = '💾 Guardar Saldos de Departamentos';
      }
